...
' Description:  Handles checkbox highlight actions
' Assumptions:  -
' Parameters:   frm - form begin checked (Form)
'               rct - name of rectangle control to turn on
' Returns:      True if no checkboxes are checked --> rectangle should be visible/displayed
'               False if checkboxes are checked --> rectangle should NOT be visible/displayed
' Throws:       none
' References:   none
' Source/date:
...
' ---------------------------------
Public Function GetCheckboxHighlight(frm As Form) As Boolean ', rct As Rectangle) As Boolean
On Error GoTo Err_Handler

    'check if any form checkboxes are set
    Dim ctrl As Control
    Dim cbxChecked As Boolean
    ...
    If cbxChecked Then
        'rct.Visible = False
        GetCheckboxHighlight = False
    Else
        GetCheckboxHighlight = True
    End If
...
Err_Handler:
    Select Case Err.Number
      Case Else
        MsgBox "Error #" & Err.Number & ": " & Err.Description, vbCritical, _
            "Error encountered (#" & Err.Number & " - GetCheckboxHighlight[mod_App_UI])"
    End Select
    Resume Exit_Handler
End Function